...
const KYCTokenSchema = new mongoose.Schema({
  token: {
    type: String,
    required: true,
    // Removed unique constraint from here - will handle at parent level
    index: true
  },
  expiresAt: {
    type: Date,
...
const userSchema = new mongoose.Schema({
  ...
  verificationToken: String, // Add this field for email verification
  verificationTokenExpire: Date, // Add this field for email verification
  terms: {
    type: Boolean,
    default: false,
...
// Indexes - Fixed to handle null values properly
userSchema.index({ email: 1 }, { unique: true });
userSchema.index({ googleId: 1 }, { unique: true, sparse: true });
userSchema.index({ 'kyc.applicantId': 1 }, { unique: true, sparse: true });
// 🔥 FIXED: Simple index without unique constraint to avoid null conflicts
userSchema.index({ 'kyc.verificationToken.token': 1 }, { sparse: true });
